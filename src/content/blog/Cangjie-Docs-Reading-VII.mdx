...
如果你要再通过这个最终结果访问其他数据, 即使它是`Some(value)`是`Option<T>`类型的, 后面要不要使用`?`操作符, 要看`value`的实际类型是否也是`Option`的, 如果是才需要用, 如果不是就不需要用. 这是规则1的内容

还有就是, 无论Optional chaining有多长, 最终的结果也就只有一层`Option`, 即 `a?.b?.c?[2]?()` 也就只有一层`Option`, 不是`None`就是`Some(value)`
